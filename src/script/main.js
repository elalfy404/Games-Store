...
forEach(document.querySelectorAll('.menu_item li.nav-dropdown'), (index, value) => {
...
const listItems = document.querySelectorAll('.update_news_section .tabs-1 span');
...
const toggleAll = (dataClass) => {
    if (dataClass === 'all') {
        ...
        for (let i = 0; i < all.length; i++)
            all[i].dataset.class === dataClass ? all[i].style.display = 'block' : all[i].style.display = 'none';
    }
};
//   
